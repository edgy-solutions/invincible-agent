...
## A THIRD MUTATION TARGET — the ORACLE, added 2026-08-29

The two targets above are the **code under the seal** (break the mechanism, watch the seal redden)
and the **fault point in a sequence** (inject at each, because one point hides a class). This adds
a third, met on the anchor amendment and not covered by either.

> **Two independent derivations agreeing is evidence only if you have shown they *can* disagree.**

### The confound, which is why the first two targets miss it

A correct system and a **coupled** system produce **identical output**. Mutating the code finds a
broken mechanism; mutating the fault point finds a hidden branch. Neither finds a test whose
*expected* value and *observed* value came from the same source — because in that case nothing is
broken, nothing is hidden, and the green is real and means nothing.

The measured instance. The anchor amendment expects symbolic values — `ANCHOR`, `ANCHOR-1`,
`ANCHOR+1` — which the **runner** resolves from the wall clock against the router's period table.
The **filler**, server-side, independently resolves *"this quarter"* to a period. Both said
`FY26-Q4`. Six of six.

**That result is exactly what a coupled harness would also produce**, and no amount of staring at
it separates the two readings. It is [row 4 of the four-row acceptance table](../plans/slot-resolution-entities-in-the-resolver-substrate.md)
wearing a calendar: *passes, and would keep passing if the thing under test were never built.*

### The move: diverge the oracle, require disagreement

Re-run with the oracle forced off the system's answer — `--anchor FY27-Q2`:

```
  WRONG 4, CORRECT 2
  D05  expected FY27-Q2   got ["FY26-Q4"]     <- the filler ignored the runner
  D08  expected FY27-Q3   got ["FY27-Q1"]
  D09/D10  still {}                            <- anchor-INDEPENDENT by construction
```

**The four positives failing is the pass.** It proves the filler answers from its own source, so
the clean run is *agreement between independent derivations* rather than an echo.

And the two negatives staying green **checks the check**: `"recently"` and `"soon"` should be
unaffected by any anchor, so a falsification that moved them would have shown the forced anchor
was perturbing something it had no business touching.

### WHEN THIS APPLIES — and the trigger is mechanical

> **Whenever an expectation is COMPUTED rather than literal.**

A literal expectation (`"FY26-Q4"` typed by a human) is independently authored and cannot be
coupled — but it goes stale the moment the wall clock crosses a boundary, and the failure reads as
a system regression. The anchor corpus chose symbolic expectations **precisely to avoid that
staleness**, and its own envelope says so: *"a corpus that hardcodes FY26-Q4 becomes wrong the day
the wall clock crosses a quarter boundary, and the failure would read as a filler regression."*

**The coupling risk is the price of that choice.** Computing the expectation buys durability and
sells independence, and the falsification is how you buy the independence back. So the rule pairs
with the design: *if you compute an expectation, you owe a run that forces it to diverge.*

Same for any shared source between oracle and system — a common constant, a config both read, a
fixture generated by the code under test.

### NOT the same as a positive control, and the distinction is load-bearing

The neighbouring practice is **non-vacuity**: prove the mechanism fired at all. Two instances in
this repo, both real and both a *different* confound:

| instance | what it proves |
|---|---|
| `[[engine-d-durability-witness]]` — `DA_SEAL = 1` on **both** sides | the replay fired in the after-run too; the observation is live |
| `[[approval-bypass-bpmn-runner]]` — asserting the decider **was consulted** on the accept path | the mechanism was exercised, not skipped |

Those answer *"did anything happen?"* This answers *"did two things happen independently?"* — and
