...
| ~~(new) authz type `disposition_item`~~ — **RETIRED, reconciled onto existing `task_audience`** | Topaz `task_audience` key `pcn_disposition:<compartment>` (`task_grants.yaml`) | Reading work's rails deleted the bespoke type + rego: a grouped review IS a HITL task, and `task_audience` already answers "who may act on a class of HITL tasks in a compartment". Single-authz-decider on the entitlement plane; died as a diff, never deployed. |
