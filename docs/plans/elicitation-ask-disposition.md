...
> **⛔ SUPERSEDED 2026-08-30 BY THE BOUND CORRECTION — the table below was true for one day.**
> The menu bound was ruled at 8 while its own worked example was *"nine capabilities is a
> menu"*; the probe (`[[enumerate-probe-2026-08-30]]`) measured that the contradiction cost
> `capability_id` its menu, and the bound is now **10**. So:
>
> | | shape | menu at bound 8 | **menu at bound 10** |
> |---|---|---|---|
> | `H06` `capability_id` (9) | elicitation | no | **YES — a real 9-option menu** |
> | `E05` `project_id` (14) | disambiguation | no | no — still `too_many`, correctly |
>
> **"Both live ask cases fall to free text" is now false**, and one of the two is a genuine
> menu the moment the fan-out lands. The claim is left visible with its correction rather
> than rewritten, because it was load-bearing in two commits and the record should show what
> moved it: a measurement, not a preference.

The superseded table, as it stood at bound 8:
