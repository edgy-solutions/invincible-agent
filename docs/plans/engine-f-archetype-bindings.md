...
3. **`fin_variance_analysis` → RULED 2026-08-30: THE VERB RETURNS THE TREE.** Build the
   hierarchical archetype; flattening is refused. **Not an amendment to Decision 3 — a
   clarification of it:** *"one verb, not a chain"* means one INVOCATION, not one STEP, and
   Decision 3 already says *"the recursion is the verb's implementation, not the caller's
   problem."* What it refuses is a CONVERSATIONAL chain. My earlier note that this "must be
   argued against Decision 3" was over-cautious and is withdrawn.
   **Do not cite the ruling by letter** — it arrived as "Option B" while this packet's (b) is
   flattening, so the letter inverts it. The words are the ruling.
   *Why flattening is actively wrong, not merely different:* the finance group performs the
   recursion themselves, so a flat verb discards analysis the engine already did and returns
   less than its inputs held.
