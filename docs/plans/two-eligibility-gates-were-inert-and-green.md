...
blocked-on:
closed-by:  9d25db7
closed-by-note: 9d25db7 repaired BOTH findings this packet names — the arity/required_args carry (mesh_registrar/main.py, utils/mesh_registration.py) and the empty PersonaTarget enum at /plan (ontology_service/main.py) — and created this packet. The LIVE VERIFICATION is a separate commit and deliberately so: a7ea68c records the post-prime measurement (arity carriage 0/10 -> 1/10 then 4/14 across four subject classes, the gate dropping exactly the derived-single verbs, /plan returning PORTFOLIO_LEAD with degraded=None). f417de6 generalised the one-verb probe into the arity_for derivation and 6abb6d3 added roll-litany leg 6 after a roll passed five legs with the engine UNREGISTERED. Citing the fix rather than the last commit to touch the file, because the closure is the repair; the rest is evidence and follow-on.
