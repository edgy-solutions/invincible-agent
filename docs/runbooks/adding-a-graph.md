...
| I wrote *"see the COPY in `Dockerfile.agent`"* in the error message. **There was no such file** — it was generated in a heredoc inside `.github/workflows/build-containers.yml`. Since 2026-09-19 it IS a file, `.github/docker/Dockerfile.agent`, moved when the heredoc crossed GitHub's 21000-char expression limit and stopped the workflow parsing. The lesson stands and its example inverted: the citation was unresolvable then and resolves now, which is why a citation needs checking rather than remembering | A citation to a nonexistent file is worse than none: it looks checkable and fails silently. Same defect as citing a transcript that does not exist |
