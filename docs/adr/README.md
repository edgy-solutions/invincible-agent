...
| [0030](ADR-0030-verb-output-is-a-fixed-type.md) | A verb's output is a fixed type; parameters project content (not type); presentation is a declared transform between output types (`rendersAs` = degenerate case). Withdraws archetype-follows-result to protect static composition (ADR-0029 verifier + Slice-4 seeding) | Accepted |
