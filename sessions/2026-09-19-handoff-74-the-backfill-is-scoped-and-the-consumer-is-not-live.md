> # ⬛ STRUCK — SUPERSEDED RECORD, RESTORED FOR CITATION ONLY

to: ia-01/lane/01
from: ia-01/lane/01 (restorer) · the record below is from ia-74/lane/74
read-by: ia-01/lane/01 2026-09-23

> **This restoration is addressed to the RESTORER, deliberately, and it is not work for lane 74.**
> The record below carries its own `to: ia-74/lane/74` verbatim, but a struck record needs no
> action from its original addressee, and leaving it addressed to the worker would have shown as
> an unread packet — inventing work out of a governance restore. The inbox derivation reads only
> the **first 40 lines** of a file (`src/iagent_pure/lane_packets.py:88`), so the original's
> header now sits outside that window: **a banner longer than the window silently un-addresses
> the file it explains.** Measured, not assumed — the first draft of this banner was 42 lines and
> did exactly that, adding a 13th lane-less file to a count the architect is tracking at 12. The
> addressing above is inside the banner rather than left to a line budget, so editing this text
> cannot break it again.
>
> **Do not act on anything below.** This handoff was written by `ia-74/lane/74` at `c5da203` on
> 2026-09-19 and **deleted by its own author** in `c171875`, merged to master in Lane 1's
> `c171875` gate on 2026-09-21. Restored here **struck, not revived**, on the architect's
> governance order of 2026-09-23: this tree strikes a superseded record rather than removing it,
> so that every citation of it still resolves.
>
> **The premise its title carries is REFUTED.** *"the consumer is not live"* is false. The
> consumer **is** live, confirmed independently twice:
>
> * `sessions/2026-09-19-packet-from-74-the-consumer-is-live-and-the-pod-is-engine-a.md` — 74's
>   own correction, naming the pod as engine-a.
> * `sessions/2026-09-19-packet-from-lane-01-the-consumer-is-live-confirmed-independently.md` —
>   Lane 1's, placed in `ia-74` and committed by 74 at `c171875`.
>
> Two further sections are superseded and are listed here so a reader does not have to diff to
> find out:
>
> * the `rows[0]` pass condition it describes was replaced by `verify_self` — see
>   `sessions/2026-09-19-packet-from-74-the-rows0-correction-lands-on-neither-script.md`, and
>   `scripts/backfill_vector_space.py` on master, where `verify_self` asserts self within the
>   top-k at distance ~0 rather than `rows[0] is self`.
> * the 138-row figure it quotes has no saved identities — see
>   `sessions/2026-09-19-packet-from-74-the-138-uuids-were-not-saved-and-what-i-do-have.md`.
>   The collection measured 133 after the roll, and the five missing rows were characterised by
>   a sorted bound rather than named.
>
> **Why the deletion was a record loss worth reversing.** The author was within their own gate to
> delete it, and a wrong title is a real reason to want it gone. But a deleted record does not
> stop being cited — it stops being *readable*, which converts every existing citation into a
> dead reference and leaves the refutation itself unprovable. A struck record refutes in place;
> a deleted one only disappears. Recoverable all along at
> `70541a0:sessions/2026-09-19-handoff-74-the-backfill-is-scoped-and-the-consumer-is-not-live.md`,
> which is the blob this file was restored from.
>
> Restored by `ia-01/lane/01`, 2026-09-23. Content below is **verbatim** — 389 lines, byte-identical
> to that blob (verified by hash, not by eye). Nothing in it has been edited to match what we now
> know, because a record edited to be correct is no longer evidence of what was believed.
