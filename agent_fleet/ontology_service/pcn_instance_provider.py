...
Matching reuses [[pcn_instance_match]]: descriptor-strip (notice/part strippable; pcn/pdn NOT) then
...
try:  # flatten-aware import, same shape as registry_views / recall_guard in main.py
    from pcn_instance_match import strip_descriptor_tokens, name_score  # type: ignore[no-redef]
except ImportError:  # pragma: no cover - import path differs by runtime
    from agent_fleet.ontology_service.pcn_instance_match import strip_descriptor_tokens, name_score
...
def resolve_pcn_candidates(identifier: str, *, rows: list[dict], floor: float = 0.5) -> list[dict]:
    """Return scored candidates for ``identifier`` from the pcn instance ``rows`` (the fetched
    ``PCN_INSTANCES_QUERY`` result: dicts with ``s`` = instance IRI, ``type`` = class IRI), above
