...
same real-shaped fixture as the seam-diff seal. The seams swap to live adapters at deploy:

  * ``resolve_subject`` -> ``resolve_subject_via_engine_o`` (LIVE; exists).
  * ``load_rules``      -> DEPLOY-GATED. Rules must come from the GRAPH (policy is the ingested TTL in
    SUSTAINMENT, changeable without a deploy — source-authority), but engine-o exposes no rules-fetch
    endpoint yet. DECISION SURFACED: add engine-o ``POST /pcn_disposition_rules`` (runs the CONSTRUCT +
    ``load_disposition_rules``, returns ``{ruleset, category_classes, ruleset_ref}``) — the pattern-
    consistent choice (engine-o owns the graph + read-union), but it means an engine-o roll too.
  * ``can_act``         -> DEPLOY-GATED. ``core/authz.py`` is decorator-shaped; a ``can_act(approver,
    item)`` predicate needs its Topaz resource_type/action DEFINED ("approver may act on a pcn
    disposition item"). DECISION SURFACED — do not invent the resource/action.
...
# ---------------------------------------------------------------------------
def load_rules_via_engine_o() -> tuple:  # pragma: no cover - deploy-gated (endpoint pending decision)
    """Fetch (ruleset, category_classes, ruleset_ref) from the GRAPH via engine-o. Requires engine-o
    ``POST /pcn_disposition_rules`` (surfaced decision) — source-authority: rules follow the ingested
    graph, never a file on disk, so a policy change without a deploy is honoured."""
    import requests
    resp = requests.post(f"{ENGINE_O_URL}/pcn_disposition_rules", json={}, timeout=_HTTP_TIMEOUT)
    resp.raise_for_status()
    body = resp.json()
    return body["ruleset"], body["category_classes"], body["ruleset_ref"]


def can_act_via_topaz(approver: str, item) -> bool:  # pragma: no cover - deploy-gated (resource/action pending)
    """Topaz predicate for grouped_review — DEPLOY-GATED. The resource_type/action for "approver may
    act on a pcn disposition item" must be DEFINED before this ships (surfaced decision); do not invent
    it. grouped_review fails CLOSED on a missing can_act, so an unwired default is safe-but-empty."""
    raise NotImplementedError(
        "can_act_via_topaz: define the Topaz resource_type/action for pcn disposition review first "
        "(surfaced decision) — see pcn_review_starter module docstring"
    )
