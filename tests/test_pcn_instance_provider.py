...
Run:  cd agent_fleet/restate_analyst && uv run --frozen --with pytest pytest ../../tests/test_pcn_instance_provider.py -v
"""
...
from agent_fleet.ontology_service.pcn_instance_provider import resolve_pcn_candidates  # noqa: E402
...
def test_exact_mpn_resolves_to_its_component_node():
    cands = resolve_pcn_candidates("NSR01L30NXT5G", rows=_ROWS)
...
def test_descriptor_wrapped_mpn_resolves():
    ...
    cands = resolve_pcn_candidates("the part NSR01L30NXT5G", rows=_ROWS)
    assert cands[0]["label"] == "NSR01L30NXT5G" and cands[0]["score"] == 1.0


def test_exact_notice_resolves_to_its_typed_node():
    cands = resolve_pcn_candidates("IPCN25300X", rows=_ROWS)
