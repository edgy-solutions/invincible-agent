...
def test_resolve_subject_extracts_instance_id_from_provenance(supervisor_mod):
    ...
    assert len(result) == 4, (
        f"Tier-3 fix changes _resolve_subject's return tuple to length 4 "
        f"(adds instance_id). Got tuple of length {len(result)}: {result!r}"
    )
    subject_uri, confidence, reasoning, instance_id = result
