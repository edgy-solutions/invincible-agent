...
#: A tracked file NOTHING ELSE IN THE SUITE WRITES, which is the load-bearing property.
#:
#: ⛔ THIS WAS `docs/BOARD.md` AND THAT MADE THE SEAL ORDER-DEPENDENT. `test_board_drift`
#: regenerates the board mid-suite, so this seal passed alone and FAILED in a full run --
#: its floor saw a dirty victim, and its main assertion saw a child whose baseline already
#: included the dirt, so the guard correctly stayed silent and the seal read that as a
#: failure to fire. Two of master 40 reds, found by the eo lane running the whole suite.
#:
#: The irony is the finding: a seal against tree mutation, defeated by tree mutation it did
#: not anticipate. `_TEMPLATE.md` is tracked, stable, and regenerated by nothing.
_VICTIM = _REPO / "docs" / "runbooks" / "_TEMPLATE.md"
...
def test_A_RUN_THAT_MUTATES_A_TRACKED_FILE_EXITS_NONZERO():
    ...
    **The floor above caught it** — this asserts the probe PASSED before reading its exit code.
    ...
    r = _run_child("\n".join([
        ...
        "    p = Path(__file__).resolve().parents[1] / 'docs' / 'runbooks' / '_TEMPLATE.md'",
    ...
    assert "_TEMPLATE.md" in r.stdout, "the guard did not name WHICH file it was"
