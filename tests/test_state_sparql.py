...
is caught here in milliseconds. Any new SPARQL template belongs in state_sparql with a test here.

Needs rdflib:  cd agent_fleet/restate_analyst && uv run --frozen --with pytest --with rdflib pytest ../../tests/test_state_sparql.py -v
"""
...
from agent_fleet.ontology_service.state_sparql import (  # noqa: E402
    build_item_state_update, build_instances_by_property_query,
)
...
def test_update_parses():
    ...
    upd = build_item_state_update(_S, "dispatchQualification", "IPCN25300X:NSR01L30NXT5G", "rules@v1")
    parseUpdate(upd)


def test_update_without_ruleset_parses():
    parseUpdate(build_item_state_update(_S, "archive", "IPCN25300X:X"))


def test_update_escapes_quotes_without_breaking():
    upd = build_item_state_update(_S, 'weird"state', 'r"ef', 'rule"set')
    parseUpdate(upd)  # escaped quotes must not break the literal / the grammar


def test_query_prepares():
    from rdflib.plugins.sparql import prepareQuery
    prepareQuery(build_instances_by_property_query("dispatchLTB"))


def test_update_writes_only_to_the_instances_graph():
    upd = build_item_state_update(_S, "dispatchLTB", "N:X")
