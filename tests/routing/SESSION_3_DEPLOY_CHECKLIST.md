...
| LLM endpoints (`OLLAMA_BASE_URL`, `MEM0_OLLAMA_BASE_URL`) | sandbox uses ai1 (`192.168.1.126`) for large models and ai2 (`192.168.1.169`) for embeddings + mem0 LLM; work uses its own LLM hosts | Predict: different IPs in work-values; benchmark for parity per `MODEL_COMPARISON_BENCHMARK.md`. **Host reservation rule (2026-06-21):** `.179` and `.188` are RESERVED for goat-yard-archive production work and MUST NOT appear in this project's values files. |
